-- Add email columns to user_stats and scan_history tables
-- This allows easy identification of users when viewing these tables in Supabase
-- Email column will be positioned as the first column (leftmost)

-- Step 1: Add email column to user_stats table (if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE public.user_stats ADD COLUMN email TEXT;
    COMMENT ON COLUMN public.user_stats.email IS 'User email address for easy identification';
  END IF;
END $$;

-- Step 2: Add email column to scan_history table (if it doesn't exist)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT FROM information_schema.columns 
    ...
    ALTER TABLE public.scan_history ADD COLUMN email TEXT;
    COMMENT ON COLUMN public.scan_history.email IS 'User email address for easy identification';
  END IF;
END $$;

-- Step 3: Backfill email for existing user_stats records
UPDATE public.user_stats us
SET email = COALESCE(
  (SELECT email FROM auth.users WHERE id = us.user_id),
  (SELECT email FROM public.users WHERE id = us.user_id)
)
WHERE us.email IS NULL;

-- Step 4: Backfill email for existing scan_history records
UPDATE public.scan_history sh
SET email = COALESCE(
  (SELECT email FROM auth.users WHERE id = sh.user_id),
  (SELECT email FROM public.users WHERE id = sh.user_id)
)
WHERE sh.email IS NULL;

-- Step 5: Reorder user_stats table to have email as first column
DO $$
DECLARE
...
-- Step 6: Reorder scan_history table to have email as first column
DO $$
DECLARE
  email_is_first BOOLEAN := FALSE;
BEGIN
  ...
  IF EXISTS (
    ...
    -- Recreate trigger function and trigger for keeping latest scans
...
-- Function to update email in user_stats when user is created/updated
...
-- Function to update email in scan_history when user is created/updated
...
-- Create triggers to sync email when auth.users is updated
...
-- Function to populate email when inserting into user_stats
...
-- Function to populate email when inserting into scan_history
...
-- Create triggers to auto-populate email on insert
